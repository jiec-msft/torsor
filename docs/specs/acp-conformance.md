...
The native boundary reuses MIT-licensed [`koffi` 3.2.0](https://www.npmjs.com/package/koffi/v/3.2.0) public FFI / struct APIs to bind [documented Job Object APIs](https://learn.microsoft.com/windows/win32/procthread/job-objects), rather than maintaining a native addon. Its platform-specific prebuilt packages support Windows x86/x64/ARM64; the Windows ownership child and Windows/Linux CLI artifact publisher load it on demand. Hosted Windows runs timed out during PowerShell `Add-Type` compilation, so runtime compilation and the PowerShell guardian are no longer required. Missing native bindings fail explicitly, without degrading to PID-tree traversal that can miss orphans.
...
CLI exit codes: `0` all passed; `1` scenario failure; `2` config, usage, or artifact I/O error; `3` skipped because real execution was not enabled. Human output contains scenario IDs, steps, and fixed diagnostics only. `--out` writes per-scenario `<id>.artifacts/` bundles without overwriting any existing destination. No automatic real-provider installation, login, authentication, or retry.

### 2.1 Recoverable artifact publication

The prerelease bundle layout cleanly replaces flat files without migrating or deleting old files. Consumers read only `result.json`, `transcript.jsonl`, and `manifest.json` in final `<id>.artifacts/`. The manifest contains `schemaVersion: 1`, `kind: "acp-artifact-bundle"`, scenario `id`, random `commitId`, and each data file's UTF-8 byte count and SHA-256; no paths, environment, raw protocol content, or machine identity. A single atomic directory publication constitutes commit; hidden staging directories are never artifacts.

Each attempt exclusively creates `.acp-artifact-<id>-<UUID>/` inside the output directory, containing `claim.json` and `bundle/`. Exclusively create the bundle's three hidden files, write and sync an immutable claim, then write and sync the three files. The claim is at most 8 KiB, with version 1, `kind: "acp-artifact-attempt"`, `state: "staging"`, scenario ID, UUID, fixed destination basename, and device/file identities of the attempt directory, claim, bundle, and three files; no fields interpreted as arbitrary paths. Hold a nonblocking exclusive OS lock on the claim throughout the attempt. Process termination releases the lock; ownership never relies on PID, clock, TTL, or filename guesses.

Publish the fully staged bundle in one same-filesystem, no-replace directory rename: Windows `MoveFileExW` without replace/copy flags, Linux `renameat2(RENAME_NOREPLACE)`. Never reserve final paths or fall back to overwrite-capable Node rename. Any existing destination file, directory (including empty), symlink, or junction is preserved with exit `2`. This slice supports Windows/Linux local filesystems with these primitives and stable file identities; missing primitives, other platforms, and unsupported filesystems fail explicitly. Files are synced, as are relevant directories on Linux; network filesystems, power-loss disk durability, and cross-scenario transactions are not guaranteed.

Before creating an attempt, each ordinary run recovers same-scenario old attempts, scanning at most 4096 direct children and processing 128 candidates; excess or unidentifiable residue is preserved with a fixed warning. Recovery first obtains the same OS lock, then revalidates strict canonical JSON, version/state/ID/UUID matching the directory name, claim handle/path identity, matching identities for every directory and present file, and absence of links or extra entries. Missing entries only accommodate interrupted cleanup. Malformed, oversized, copied, unknown-version, or identity-mismatched claims cannot authorize deletion; a reserved filename alone is insufficient.

Cleanup individually unlinks verified staged files, then removes the empty bundle, claim, and empty attempt directory; never recursively deletes, follows links, or modifies a final bundle. Recovery cannot steal a live writer's lock; concurrent recoverers revalidate and only the lock holder cleans. Interruption before a complete claim can leave unprovable residue, which must be preserved; a new UUID keeps it from blocking retry. Write, sync, close, or cleanup failures explicitly return an I/O error; known post-commit errors explicitly identify the committed bundle. Revalidate staging identities and the complete file set before publication, rejecting observable external changes. After interruption at any pre-publication step, an ordinary CLI retry can commit a complete bundle. After publication but before acknowledgement or cleanup, the final bundle is already committed: retry recovers hidden residue only and refuses overwrite. Consumers never observe a harness-published partial data pair.
...
Parsing/resource failures from deeply nested YAML flow sequences, flow mappings, and block mappings must promptly return sanitized `ConfigurationError`, without exposing raw parser exceptions, source, or values. Production dependencies use `yaml` 2.9.1 (ISC) and `ajv` 8.20.0 (MIT), outside the reported advisory ranges for GHSA-48c2-rrv3-qjmp and GHSA-2g4f-4pwh-qvx6 respectively; Ajv `$data` is not enabled. Parser upgrades must preserve existing size, depth, and JSON-compatibility limits.
