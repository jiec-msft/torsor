...
8. Operational logging is not a safety gate. Physical stop or containment for cancellation, Writer Authority loss, Lease expiry, shutdown, or an unknown stop must start first and must not wait on a fallible log write. Logging failure still propagates explicitly, but cannot prevent or replace the OS stop, durable stop/quarantine evidence, or independently mask a stop failure.
...
1. The HTTP boundary creates a `requestId` for every request. `http.request` always records it. When a command commits, that event also records the Kernel-returned `correlationId`. A successful command response may expose 2xx only after the required event has been accepted by the sink, and returns the same `correlationId`, allowing an operator to enter the durable work chain from the response-header `requestId`. If logging fails after commit, the Host fails explicitly and the client must not observe 2xx; the client may treat the command outcome as uncertain and recover with the original idempotency key.
...
3. Runtime obtains the original `correlationId` from the Attention or triggering RunInput creation event. An initial RunInput created by the same Attention command, with no independent public creation event, inherits the Run creation event correlation. Runtime uses that value as server-bound operation context for Attention and Run Activation, ProviderAttempt, Agent capability, native execution, and settlement; the corresponding durable Kernel events and operational events must retain the same verified correlation. Prompts, Provider output, and HTTP bodies cannot supply or override it.
