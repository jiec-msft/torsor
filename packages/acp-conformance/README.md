...
`--out <directory>` writes per-scenario `<id>.artifacts/` containing `result.json`, `transcript.jsonl`, and `manifest.json` with version, scenario ID, commit UUID, data-file byte counts and SHA-256, refusing overwrites. Results are `passed`, `failed`, or `skipped`. CLI codes are all-passed `0`, failure `1`, configuration/usage/I/O error `2`, and real-provider-disabled `3`; failure takes precedence over skips.

The bundle is the sole consumer commit boundary, replacing flat files without migrating/deleting them. Stage and sync complete content in a unique hidden attempt directory, then publish with a Windows/Linux same-filesystem atomic no-replace directory rename. Any existing destination, including an empty directory, file, symlink, or junction, remains unchanged. Never consume hidden staging directories.

Ordinary retries first recover known dead-writer staging using an exclusive OS lock and a strict, bounded, versioned claim bound to file identities, never PID/time or filename-only deletion. Cleanup only removes verified staged files and empty directories, never follows links or modifies final bundles. Live writers, unknown/malformed claims, extra files, and identity-changed residue are preserved; unverifiable residue produces a fixed warning, but unique attempt names keep it from blocking a fresh commit. Pre-publication termination permits normal retry; after publication but before acknowledgement, the complete bundle is committed and retry preserves it and refuses overwrite. I/O/cleanup failures are explicit. Windows/Linux local filesystem native locks, stable file identities, and no-replace rename are required; no overwrite-capable fallback, network-filesystem, power-loss, or cross-scenario atomicity guarantee. See [specification §2.1](../../docs/specs/acp-conformance.md#21-recoverable-artifact-publication) for the complete boundary.
